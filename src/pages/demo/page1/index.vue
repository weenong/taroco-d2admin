<template>
  <d2-container>
    ...
    <div class="wraper" ref="wraper">
        <div class="canvas-wraper" ref="cw">
...
<script>
  ...
  export default{
    ...
    created() {
      this.$store.commit('fabric/drawtype/TOOGLE_TOOLS', 'pencil')
    },
    mounted() {
      //初始化canvas 
      setTimeout( ()=>{
        console.log(this.$refs.wraper.offsetHeight)
        this.initCanvas()
      },16)
    },
    computed:{
    ...
    methods:{
      ...
      handleTools(tools, idx) {
        this.initIdx = idx;
        this.$store.commit('fabric/drawtype/TOOGLE_TOOLS', tools.name)
        if(tools.name != 'reset') {
          this.drawing()
    ...
    watch:{
      '$store.state.fabric.drawtype.drawType': {
        handler: function() {
          this.currentTool = this.$store.state.fabric.drawtype.drawType
        },
        deep: true
      }
    }
